...
#include "SurfaceGenerator/include/SimpleSurface.hh"
...
G4LogicalVolume *Surface::SimpleSurface::GetRoughSurface() {
  SurfaceGenerator generator = SurfaceGenerator("SimpleSurface");
  ...
  G4Box *basisSolid =
      new G4Box("Surface::Basis", 0.5 * sizeX, 0.5 * sizeY, 0.5 * basisSizeZ);
  const G4ThreeVector basisTranslation{0, 0, -0.5 * basisSizeZ};
  G4Transform3D basisTransform{G4RotationMatrix(), basisTranslation};
  munionSolid->AddNode(*basisSolid, basisTransform);

  G4Voxelizer_Green &voxelizer =
      (G4Voxelizer_Green &)(munionSolid->GetVoxels());
  voxelizer.SetMaxBoundary(10, 10, 2);
  voxelizer.Voxelize(munionSolid);

  G4LogicalVolume *munionLogic =
      new G4LogicalVolume(munionSolid, roughnessMaterial, "Surface::Patch");

  G4ThreeVector boundingMin, boundingMax;
  munionSolid->BoundingLimits(boundingMin, boundingMax);
  const G4double subworldSizeZ = boundingMax.z() - boundingMin.z();
  G4Box *subworldSolid = new G4Box("Surface::Subworld", 0.5 * sizeX,
                                   0.5 * sizeY, 0.5 * subworldSizeZ);
  G4LogicalVolume *subworldLogic =
      new G4LogicalVolume(subworldSolid, subworldMaterial, "Surface::Subworld");

  const G4ThreeVector roughnessPlacement = {0, 0,
                                            -0.5 * subworldSizeZ + basisSizeZ};
  new G4PVPlacement(0, roughnessPlacement, munionLogic, "Surface::Roughness",
                    subworldLogic, false, 0, 0);
