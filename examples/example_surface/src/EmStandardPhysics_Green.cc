...
//  date: 12.03.2024
//
// header: EmStandardPhysics_Green.hh
...
void EmStandardPhysics_Green::ConstructProcess() {
  ...
  for (const auto &particleName : partList.PartNames()) {
    ...
    if (particleName == "gamma") {
      ...
      NRmsc->SetEmModel(model, 1);
      ph->RegisterProcess(NRmsc, particle);
      ...
      G4ScreenedNuclearRecoil *nucrec =
          new G4ScreenedNuclearRecoil("ScreenedElastic", "zbl");
      nucrec->SetMaxEnergyForScattering(energyLimit);
      ph->RegisterProcess(nucrec, particle);
